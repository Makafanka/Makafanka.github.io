...
Our dataset is sourced from the [Video Game Sales dataset from Kaggle](https://www.kaggle.com/datasets/gregorut/videogamesales/data), which was generated by scraping data from vgchartz.com [^2]. The dataset contains 16,598 video games across 11 distinct fields. Below is a snapshot of the dataset (first five rows):
![Figure 1: First 5 rows of the dataset](assets/Figure1.png)

The fields in the dataset include:
* Sale Rank: Rank of the game based on global sales.
* Game Name: Title of the video game.
* Release Platform: The gaming platform (e.g., PS4, Xbox One).
* Release Year: Year the game was released.
* Publisher: The company that published the game.
* Sales in NA, EU, JP, and Other Regions: Sales data in North America, Europe, Japan, and other regions, respectively.
...
Since the dataset does not include a “most popular area” field, we created a new column to identify the region with the highest sales. We implemented a lambda function to add this field, categorizing games into one of four regions based on their highest sales:
* North America (NA): Labeled as 1
* Europe (EU): Labeled as 2
* Japan (JP): Labeled as 3
* Other Regions: Labeled as 0

To further facilitate classification, we also encoded the categorical fields Publisher and Genre into numerical labels:
* publisher_label: A numerical label ranging from 0 to the number of unique publishers.
* genre_label: A numerical label ranging from 0 to the number of unique genres.

Besides, among the dataset fields, the Year of Release is a potentially biased term. Technological advancements and societal trends have significantly influenced gaming equipment, audience preferences, and sales patterns over time. As a result, the release year may bias the prediction of game sales across platforms and regions. This bias must be carefully considered when interpreting our model’s results.
...
For the other neural networks trained with Adam Optimizer, it reaches an accuracy of 61% after 2 epochs for training data and stays unchanged. The accuracy for validation data is about 60% and the loss for validation data is about 0. They stay unchanged for all epochs. (Figure 3)

When we use test data to test the accuracy, we get a loss of 0.7277 and an accuracy of 61%.

* RMSprop Optimizer

After about 18 epochs, the accuracy and loss remain unchanged for the training and validation data. The accuracy for the training and validation dataset reached about 60% and the loss decreased to nearly 0 (Figure 4).
